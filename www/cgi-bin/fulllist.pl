...
my $sth=$dbh->prepare("select calls.source,calls.destination,calls.start,calls.answer,calls.end,calls.duration,status.status from calls,status where calls.status=status.id order by calls.start");
